...
class GroupedDFNDoubleLayer(pybamm.lithium_ion.BaseModel):
    """
    A grouped parameter version of the Doyle-Fuller-Newmann model (DFN) with potential difference formulation.
    ...
    **model_kwargs : optional
    ...
    def __init__(self, name="Grouped Doyle-Fuller-Newmann model", **model_kwargs):
        ...
        v_sf_n = Variable(
            "Negative electrode film potential difference [V]",
            domain="negative electrode",
            scale=Scalar(0.010),
        )
        v_sf_p = Variable(
            "Positive electrode film potential difference [V]",
            domain="positive electrode",
            scale=Scalar(0.010),
        )
        ...
        sto_e_sep = Variable(
            "Separator electrolyte stoichiometry", domain="separator", scale=Scalar(1)
        )
        sto_e_p = Variable(
        ...
        i_tot_n = pybamm.Variable(
            "Negative lumped total current [A]", domain="negative electrode"
        )
        i_tot_p = pybamm.Variable(
            "Positive lumped total current [A]", domain="positive electrode"
        )
        ...
        c_hat_p_dl = Parameter("Positive electrode lumped double layer capacitance [F]")
        c_hat_n_dl = Parameter("Negative electrode lumped double layer capacitance [F]")
        ...
        ######################
        # Electrolyte current relations
        ######################
        prefactor_n = 1 / kappa_hat_n + 1 / sigma_hat_n
        prefactor_p = 1 / kappa_hat_p + 1 / sigma_hat_p

        i_e_p = (
            I / sigma_hat_p
            + pybamm.grad(v_sf_p)
            - kappa_hat_D_p * T * pybamm.grad(sto_e_p) / sto_e_p
        ) / prefactor_p

        i_e_n = (
            I / sigma_hat_n
            + pybamm.grad(v_sf_n)
            - kappa_hat_D_n * T * pybamm.grad(sto_e_n) / sto_e_n
        ) / prefactor_n

        i_tot_n = pybamm.div(i_e_n)
        i_tot_p = pybamm.div(i_e_p)

        ######################
        # Surface film voltage drop
        ######################
        self.algebraic[v_sf_n] = v_sf_n - v_s_n - r_f_n * i_tot_n
        self.algebraic[v_sf_p] = v_sf_p - v_s_p - r_f_p * i_tot_p

        bc_v_sf_p_right = -I / sigma_hat_p
        bc_v_sf_p_left = I / pybamm.boundary_value(
            kappa_hat_p, "left"
        ) + pybamm.boundary_value(kappa_hat_D_p, "left") * T * pybamm.boundary_gradient(
            sto_e_p, "left"
        ) / pybamm.boundary_value(sto_e_p, "left")

        bc_v_sf_n_left = -I / sigma_hat_n
        bc_v_sf_n_right = I / pybamm.boundary_value(
            kappa_hat_n, "right"
        ) + pybamm.boundary_value(
            kappa_hat_D_n, "right"
        ) * T * pybamm.boundary_gradient(sto_e_n, "right") / pybamm.boundary_value(
            sto_e_n, "right"
        )

        self.boundary_conditions[v_sf_p] = {
            "left": (bc_v_sf_p_left, "Neumann"),
            "right": (bc_v_sf_p_right, "Neumann"),
        }
        self.boundary_conditions[v_sf_n] = {
            "left": (bc_v_sf_n_left, "Neumann"),
            "right": (bc_v_sf_n_right, "Neumann"),
        }

        self.initial_conditions[v_sf_n] = U_n_init
        self.initial_conditions[v_sf_p] = U_p_init

        ######################
        # Double layer
        ######################
        self.rhs[v_s_p] = (i_tot_p - i_f_p) / c_hat_p_dl
        self.rhs[v_s_n] = (i_tot_n - i_f_n) / c_hat_n_dl
        ...
        x_sep = pybamm.standard_spatial_vars.x_s
        x_p = pybamm.standard_spatial_vars.x_p

        i_s_n = I - i_e_n
        phi_s_n = pybamm.IndefiniteIntegral(
            -i_s_n / sigma_hat_n, x_n
        )  # phi_s_n at x=0 is 0
        phi_e_n = phi_s_n - v_sf_n
        phi_e_n_sep = pybamm.boundary_value(phi_e_n, "right")

        i_e_sep = pybamm.PrimaryBroadcastToEdges(I, "separator")
        phi_e_sep = phi_e_n_sep + pybamm.IndefiniteIntegral(
            - i_e_sep / kappa_hat_sep
            - kappa_hat_D_sep * T * pybamm.grad(sto_e_sep) / sto_e_sep,
            x_sep,
        )

        i_s_p = I - i_e_p
        phi_s_p = (
            pybamm.boundary_value(phi_e_sep, "right")
            + pybamm.boundary_value(v_sf_p, "left")
            + pybamm.IndefiniteIntegral(-i_s_p / sigma_hat_p, x_p)
        )
        phi_e_p = phi_s_p - v_sf_p
        ...
        ######################
        self.rhs[sto_e_n] = (
            (psi_hat_n * T * pybamm.div(kappa_hat_n * pybamm.grad(sto_e_n)) + i_tot_n)
            / 3600
            / q_e_n
        )
        self.rhs[sto_e_sep] = (
            (psi_hat_sep * T * pybamm.div(kappa_hat_sep * pybamm.grad(sto_e_sep)))
            / 3600
            / q_e_sep
        )
        self.rhs[sto_e_p] = (
            (psi_hat_p * T * pybamm.div(kappa_hat_p * pybamm.grad(sto_e_p)) + i_tot_p)
            / 3600
            / q_e_p
        )

        kappa_ratio_n_sep = pybamm.boundary_value(
            kappa_hat_n, "right"
        ) / pybamm.boundary_value(kappa_hat_sep, "left")
        kappa_ratio_p_sep = pybamm.boundary_value(
            kappa_hat_sep, "right"
        ) / pybamm.boundary_value(kappa_hat_p, "left")
        self.boundary_conditions[sto_e_n] = {
            "left": (Scalar(0), "Neumann"),
            "right": (pybamm.boundary_value(sto_e_sep, "left"), "Dirichlet"),
        }
        self.boundary_conditions[sto_e_sep] = {
            "left": (
                pybamm.boundary_gradient(sto_e_n, "right") * kappa_ratio_n_sep,
                "Neumann",
            ),
        ...
        self.boundary_conditions[sto_e_p] = {
            "left": (
                pybamm.boundary_gradient(sto_e_sep, "right") * kappa_ratio_p_sep,
                "Neumann",
            ),
        ...
        self.variables = {
            ...
            "Negative lumped total current [A]": i_tot_n,
            "Positive lumped total current [A]": i_tot_p,
            "Time [s]": pybamm_t,
            "Current [A]": I,
    ...
    def default_var_pts(self):
        ...
        return {x_n: 40, x_s: 40, x_p: 40, r_n: 40, r_p: 40}
    ...
    def create_grouped_parameters(parameter_values: ParameterValues) -> ParameterValues:
        ...
        c_hat_p_dl = Cdl_p * a_s_p * A * L_p
        c_hat_n_dl = Cdl_n * a_s_n * A * L_n
        r_hat_p_dl = Rdl_p / (a_s_p * A * L_p)
        r_hat_n_dl = Rdl_n / (a_s_n * A * L_n)
        ...
        parameter_dictionary = {
            ...
            "Negative electrode lumped double layer capacitance [F]": c_hat_n_dl,
            "Positive electrode lumped double layer capacitance [F]": c_hat_p_dl,
            "Negative electrode lumped double layer resistance [Ohm]": r_hat_n_dl,
            "Positive electrode lumped double layer resistance [Ohm]": r_hat_p_dl,
            "Negative electrolyte lumped constant for transport and thermodynamic factor [V.K-1]": kappa_hat_D,
            "Separator electrolyte lumped constant for transport and thermodynamic factor [V.K-1]": kappa_hat_D,
            "Positive electrolyte lumped constant for transport and thermodynamic factor [V.K-1]": kappa_hat_D,
            "Positive electrolyte scale ratio between lumped diffusivity and conductivity [V.K-1]": psi_hat,
            "Separator electrolyte scale ratio between lumped diffusivity and conductivity [V.K-1]": psi_hat,
            "Negative electrolyte scale ratio between lumped diffusivity and conductivity [V.K-1]": psi_hat,
