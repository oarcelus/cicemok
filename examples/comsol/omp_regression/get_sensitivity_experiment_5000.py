...
def main():
    ...
    npool = 5
    with open("samples_r_n5000_o4.pkl", "rb") as file:
        samples_r = pickle.load(file)

    with open("samples_q_n5000_o4.pkl", "rb") as file:
        samples_q = pickle.load(file)

    # Save evals for the current iteration
    with open("evaluations_n5000_o4.pkl", "rb") as file:
    ...
    pool = multiprocessing.Pool(
        processes=npool,
        initializer=pq_loo_init_worker,
        initargs=(precomputed_polynomials, init_event),
    )
    try:
        init_event.wait()
        ...
        # with open("surrogate_n5000_o4.pkl", "wb") as file:
        #     dill.dump([x, surrogate], file)
        # # Save surrogate for the current iteration
        # with open("alpha_n5000_o4.pkl", "wb") as file:
        #     pickle.dump([x, alpha], file)
        # # Save surrogate for the current iteration
        # with open("fourier_n5000_o4.pkl", "wb") as file:
        #     pickle.dump([x, fourier], file)
        #
        # if sobol_t:
        #     # Save sobol indices of the full time series
        #     with open("sobol_total_n5000_o4.pkl", "wb") as file:
        #         pickle.dump(sobol_t, file)
        #
        # if sobol_2:
        #     # Save sobol indices of the full time series
        #     with open("sobol_interaction_n5000_o4.pkl", "wb") as file:
        #         pickle.dump(sobol_2, file)
        #
        # # Save sobol indices of the full time series
        # with open("sobol_n5000_o4.pkl", "wb") as file:
