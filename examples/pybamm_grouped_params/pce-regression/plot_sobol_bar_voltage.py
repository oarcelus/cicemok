...
def main():
    ...
    for i, volt in enumerate(voltages):
        for j, name in enumerate(names):
            with open("./sobol_n5000.pkl", "rb") as file:
                vals = pickle.load(file)
                trusobol = vals[j, :]
            with open("./sobol_total_n5000.pkl", "rb") as file:
                vals = pickle.load(file)
                totaltrusobol = vals[j, :]

            ax_flat[i].bar(j - 2*offset, s1s[name][volt], bar_width, color=cmap[j], alpha=0.5)
            ...
            ax0 = ax_flat[i].scatter(
                j - bar_width/2,
                trusobol[volt],
                marker='x',
                color=cmap[j],
                label=r"$1^{st}$ Sobol (PCE)"
            )
            ax_flat[i].errorbar(
                j + offset,
                sts[name][volt],
            ...
            ax1 = ax_flat[i].scatter(
                j + 2*offset + 0.05,
                totaltrusobol[volt],
                marker='v',
                color=cmap[j],
                label="Total Sobol (PCE)"
            )
        ax_flat[i].set_xticks(range(len(names)))  # Set tick positions
        ax_flat[i].set_xticklabels([])
        ax_flat[i].set_ylim(-0.02, 1.0)
        ax_flat[i].set_title(f"Voltage: {lnsamples[volt]:.2f} V", loc="right")
    ...
    ax_flat[-1].set_xticklabels(names)
    non_hatched_patch = mpatches.Patch(alpha=0.5, facecolor="grey", edgecolor="black", label=r"$1^{st}$ Sobol")
    hatched_patch = mpatches.Patch(alpha=0.2, facecolor="grey", edgecolor="black", hatch="///", label="Total Sobol")
    ax_flat[0].legend(handles=[non_hatched_patch, hatched_patch, ax0, ax1])
    for ax in ax_flat:
