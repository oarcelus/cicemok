...
def main():
    ...
    for i in range(repetitions[0], repetitions[1]):
        for j, nsamples in enumerate(lnsamples):
            with open(f"./surrogate_n{nsamples}_o5_reps{i}.pkl", "rb") as file:
                vals = dill.load(file)
                ysurr = vals[1]
            with open(f"./fourier_n{nsamples}_o5_reps{i}.pkl", "rb") as file:
            ...
            with open(
                f"pce_generalization_error_n{nsamples}_o5_reps{i}.pkl", "wb"
            ) as file:
