...
def main():
    ...
    for i in range(repetitions):
        for j, nsamples in enumerate(lnsamples):
            ...
            fourier, surrogate, alpha, sobol, sobol_2, sobol_t = get_sa_from_experiment(
                samples=samples_r,
                y=ys,
                problem=None,
                config=sens_cfg,
            ...
            with open(f"surrogate_n{nsamples}_o{sens_cfg.order}_reps{i}.pkl", "wb") as file:
                dill.dump([x, surrogate], file)
            # Save surrogate for the current iteration
            with open(f"alpha_n{nsamples}_o{sens_cfg.order}_reps{i}.pkl", "wb") as file:
                pickle.dump([x, alpha], file)
            # Save surrogate for the current iteration
            with open(f"fourier_n{nsamples}_o{sens_cfg.order}_reps{i}.pkl", "wb") as file:
                pickle.dump([x, fourier], file)

            if sens_cfg.sobol_total:
                # Save sobol indices of the full time series
                with open(f"sobol_total_n{nsamples}_o{sens_cfg.order}_reps{i}.pkl", "wb") as file:
                    pickle.dump(sobol_t, file)

            if sens_cfg.sobol_second:
                # Save sobol indices of the full time series
                with open(f"sobol_interaction_n{nsamples}_o{sens_cfg.order}_reps{i}.pkl", "wb") as file:
                    pickle.dump(sobol_2, file)

            # Save sobol indices of the full time series
            with open(f"sobol_n{nsamples}_o{sens_cfg.order}_reps{i}.pkl", "wb") as file:
