...
def main():
    ...
    lnsamples = [256, 512, 1024, 2048, 4096, 8192, 16384]
    ...
    for nsamples in lnsamples:
        ...
        with open(f"./evaluations_n{nsamples}.pkl", "rb") as file:
            evals = pickle.load(file) 
            x = evals[0]
            evals = evals[1]
        with open(f"./samples_n{nsamples}.pkl", "rb") as file:
            samples = pickle.load(file) 

        # npevals = np.stack(evals)
        # data = (npevals[:nsamples, -1],)
        # res = bootstrap(data, np.mean, confidence_level=0.95)
        #
        # mean = np.mean(npevals[:nsamples, -1])
        # results_mean.append((mean, res))
        #
        # res = bootstrap(data, np.var, confidence_level=0.95)
        #
        # mean = np.var(npevals[:nsamples, -1], ddof=1)
        # results_var.append((mean, res))

    with open("./statistics_data.pkl", "wb") as file:
        pickle.dump([lnsamples, results_mean, results_var], file)
