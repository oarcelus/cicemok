...
def main():
    ...
    current_cfg = CurrentConfigurations(isoc=1.0, texp=5000.0*1.0, experiment=experiment)
    comsol_cfg = ComsolConfiguration(
        names=["Ds_p", "Ds_n", "k_p", "k_n", "Rc_p", "Rc_n", "Rfilm_p", "Rfilm_n", "sigma_p", "sigma_n", "tau_p", "tau_n", "Cdl_p", "Cdl_n", "cl_0"],
        filename="../../../lg_basic.mph",
        expression=["liion.phis0_ec1", "-t*I_app*0.000277778"],
        unit=["m^2/s", "m^2/s", "m/s", "m/s", "ohm*m^2", "ohm*m^2", "ohm*m^2", "ohm*m^2", "S/m", "S/m", "1", "1", "F/m^2", "F/m^2", "mol/m^3"],
    ...
    try:
        ...
        lnsamples = [2*16384]
        sens_cfg = SensitivityConfiguration(distribution=distribution, rule='sobol', sobol_second=True, sobol_total=True)
        distribution_r = cp.J(
        ...
        for nsamples in lnsamples:
            ...
            samples_r, x, ys, weights, problem = get_sampling_from_experiment(
                npool, ncores, nsamples, ninterp, comsol_cfg, sens_cfg, exclude, pool, method="salib"
            )
